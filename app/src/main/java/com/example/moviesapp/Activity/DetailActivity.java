...
public class DetailActivity extends AppCompatActivity {
    ...
    private String userId = "1"; // Thay bằng Firebase Auth nếu cần
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        favImage.setOnClickListener(v -> {
            ...
            ApiMovie.apiMovie.getFavouriteMovies(userId, 1, 100, null).enqueue(new Callback<FavouriteResponse>() {
                @Override
                        ...
                        if (isFavourite && matchedFavourite != null) {
                            ...
                            new AlertDialog.Builder(DetailActivity.this)
                                    ...
                                    .setPositiveButton("Yes", (dialog, which) -> {
                                        ApiMovie.apiMovie.removeFavouriteMovie(userId, finalMatchedFavourite.getMovieId()).enqueue(new Callback<Void>() {
                                            @Override
...
//                                                    favImage.setImageResource(R.drawable.ic_favorite_border);
                            ...
                            // Phim chưa có → thêm
                            ApiMovie.apiMovie.addFavouriteMovie(userId, request).enqueue(new Callback<Void>() {
                                @Override
                                public void onResponse(@NonNull Call<Void> call, @NonNull Response<Void> response) {
                                    if (response.isSuccessful()) {
...
//                    favImage.setImageResource(isFavourite ? R.drawable.ic_favorite : R.drawable.ic_favorite_border);
    ...
    private void sendRequest(String movieId) {
        ...
        ApiMovie.apiMovie.getMovieDetails(movieId).enqueue(new Callback<MovieDetailsResponse>() {
            @Override
